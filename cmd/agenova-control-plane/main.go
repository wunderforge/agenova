...
func handler() http.Handler {
	...
	mux.HandleFunc("GET /v1/status", func(writer http.ResponseWriter, _ *http.Request) {
		...
		_ = json.NewEncoder(writer).Encode(status{Platform: os.Getenv("AGENOVA_PLATFORM_NAME"), Revision: os.Getenv("AGENOVA_PLATFORM_REVISION"), Policy: os.Getenv("AGENOVA_POLICY_REF"), State: "available"})
	})
	return mux
}
