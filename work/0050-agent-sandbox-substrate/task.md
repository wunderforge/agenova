...
- Decision (Owner-confirmed 2026-08-31): pin upstream Agent Sandbox **`v1.0.0`** (published 2026-08-28), the current latest release — not `v0.4.6` (May 2026, used by the prior exploratory branch). `v1.0.0` serves the core and extension APIs as `v1beta1` (no `v1alpha1`); its release assets are `sandbox.yaml` (core), `extensions.yaml`, and `sandbox-with-extensions.yaml` — the script targets these, not the old `manifest.yaml` name. This is a fresh disposable cluster, so the upstream upgrade/migration path does not apply. Fall back to `v0.5.6` only if `v1.0.0` proves unstable on kind, and record that as a blocker first.
