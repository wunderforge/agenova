...
- Decision (Owner-confirmed 2026-08-31, revised same day): pin upstream Agent Sandbox **`v0.4.6`** (`extensions.agents.x-k8s.io/v1alpha1`), matching the rest of the codebase — the `internal/runtime/agentsandbox` adapter, `docs/backends/agent-sandbox.md`, `THIRD_PARTY_NOTICES.md`, and the `scripts/checks/repository.ps1` check are all on `v0.4.6`. An earlier revision of this packet pinned `v1.0.0` (`v1beta1`); the Owner reverted it to keep this substrate consistent with the shipped adapter. Assets for `v0.4.6` are `manifest.yaml` (core) + `extensions.yaml`. Bumping the whole E8 surface to `v1.0.0`/`v1beta1` is deferred and tracked by the #66 mapping spike.
