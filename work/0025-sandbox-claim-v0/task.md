...
- [x] Address owner re-review (PR #100, 2026-09-02): merged current `main` (brings in `ClaimRequest`/#24, including `Duration`'s `MarshalJSON`/`UnmarshalJSON` in `api/v1alpha1/validation.go`) via `git merge origin/main` (not rebase — the branch was already pushed and had already merged `main` once before). `ValidateIssuedState` now fails closed on blank `claim.id`, an unknown/blank `claim.phase`, blank `policyRef.id`/`policyRef.version`, blank `decision.reason`, blank `backendIdentity.backend`/`workerId`, and a `claim.templateRef`/`action.templateRef` mismatch, each covered by a table-driven negative test (`TestValidateIssuedState_FailsClosedOnBlankOrInvalidFields`). `EffectiveAuthorityRuntime.Timeout` now reuses `api/v1alpha1.Duration` instead of a parallel string + `time.ParseDuration`. `Evidence.RuntimeEvents`/`ToolInvocations`/`ModelInvocations` are normalized to a non-nil empty slice at decode time and lost `omitempty`, fixing the parse→marshal→parse drift the fixtures' explicit `[]` values previously fell into (`TestIssuedStateFixtures_RoundTrip` exercises both valid shared fixtures for semantic equivalence, not just decode success). `go test ./...` and `.\scripts\check.ps1 -All` pass.
- [x] Address owner's third-round review (PR #100, 2026-09-02, on commit `415593a`): the prior correlation-only checks (`claim.templateRef == action.templateRef`, `claim.authorityRef == effectiveAuthority.id`) passed trivially when both sides were blank. Added explicit required-field checks for `action.project`, `action.templateRef`, `effectiveAuthority.id`, and `effectiveAuthority.runtime.profileRef` so a blank-both-sides snapshot fails closed instead of correlating two empty strings. Extended `TestValidateIssuedState_FailsClosedOnBlankOrInvalidFields` with cases for each. `go test ./...` and `.\scripts\check.ps1 -All` pass.
