...
8. Build a canonical secret-free desired-state payload that explicitly excludes computed fields such as `revision`; hash that payload once, then attach the resulting revision to `ResolvedPlatform` and the inspectable lock/plan projection.
...
- Golden resolved lock/plan proves stable ordering/digest and independent deployment/runtime/model selection. Recomputing from the same canonical desired-state payload produces the same revision because computed fields are outside the digest input.
- Runtime fixtures include the canonical worker template and warm-pool resources required by the current Agent Sandbox adapter so #45 does not invent hidden bootstrap state.
