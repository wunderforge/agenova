...
2. `spec.infrastructure` and `spec.services` contain typed categories of named instances. Each instance references one adapter requirement, maps explicit profiles and carries opaque adapter-owned `config`.
3. A pure resolver validates references/capabilities/config and returns a deterministic `ResolvedPlatform`/lock projection. It has no target mutation methods.
...
- A narrow descriptor lookup interface returns only identity, version, declared capabilities and a config validator. #151 will implement it with the explicit bundled registry.
...
PlatformLock
  ...
  instances[]           # category/path, name, adapter ref, config digest, profiles
  initialPolicyRef      # reference only
```

Raw config, provider output and credentials never enter the lock. Input order is normalized for revision/plan stability; diagnostics retain field paths.
...
5. Invoke each adapter's side-effect-free config validator.
6. Validate unique profile mappings and cross-references.
7. Canonicalize, hash and emit resolved lock/plan projection.

Any error returns no resolved result and invokes no mutation seam.
...
- Descriptor/config-validator spies prove correct adapter dispatch and zero mutation on all failures.
...
- Opaque config can hide secrets unless generic reserved-key checks and adapter validators both run; tests must cover nested maps/lists and redact diagnostics.
