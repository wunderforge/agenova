...
// Request is a governed tool invocation request from an agent claim. It
// carries claim identity and scoped operation data only: no provider
// credentials and no backend types.
type Request struct {
	ClaimID       string
	Tool          string
	Action        string
...
// the claim-activity baseline built into the gateway; capability and
// resource-scope enforcement layers on through this seam (E4-T2), and an
// approval-routing policy may return ApprovalRequired without granting
// anything (approval lifecycle is owned elsewhere).
type Policy func(req Request) gateway.Outcome

// Gateway authorizes tool requests for active claims and records
...
// Invoke decides one governed tool attempt. The returned Decision carries the
...
func (g *Gateway) record(req Request, id string, out gateway.Outcome) gateway.Decision {
	g.store.RecordToolInvocation(req.ClaimID, toolName(req), id, out.Result)
...
func validate(req Request) (gateway.Outcome, bool) {
	...
	if strings.TrimSpace(req.Tool) == "" || strings.TrimSpace(req.Action) == "" {
		return gateway.Outcome{
			...
			Reason:   "tool request must identify tool and action",
		}, true
	}
	if gateway.AmbiguousResourceScope(req.ResourceScope) {
		return gateway.Outcome{
			Result:   gateway.ResultDeny,
			Category: gateway.CategoryAmbiguousResourceScope,
			Reason:   fmt.Sprintf("resource scope %q does not name one bounded resource", req.ResourceScope),
		}, true
	}
