...
func TestMultiAgentReference(t *testing.T) {
	...
	toolGW := toolgateway.NewGateway(claims, lineage, store, toolgateway.WithAdapter(toolAdapter))
	modelGW := modelgateway.NewGateway(claims, lineage, store, modelgateway.WithAdapter(modelAdapter))
...
func TestMultiAgentReference_WorkerWithoutParentIsIndependent(t *testing.T) {
	...
	store := facts.NewStore()
	lineage := governance.NewLineage()
	toolGW := toolgateway.NewGateway(legacyClaimReader{runtime: r}, lineage, store, toolgateway.WithAdapter(&referenceToolAdapter{}))
