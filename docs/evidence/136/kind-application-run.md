...
- Disposable namespace: `agenova-run136-reviewfix` (created and deleted by the test)
...
go test -count=1 -v -tags 'integration controlled' -timeout 5m `
  ...
  -namespace agenova-run136-reviewfix
```

The test requires a namespace name that does not already exist. It creates that namespace itself, retains it if lifecycle cleanup cannot be confirmed, and deletes it only after the correlated worker and claim resources are confirmed released.
...
governed kind run complete: request=fix-payment-timeout decision=Allow claim=claim:fix-payment-timeout:issuance:cb71a09da1c3a0f4ff16dd25bfb758fa worker=agenova-pool-reference-engineer-pool-99hgk phase=Succeeded
...
The test independently listed the namespace after the denied request and after application cleanup. No `SandboxClaim` remained, and both the released Sandbox and Pod identities were absent. The integration-owned namespace, including its template and warm pool, was then deleted by the test.
