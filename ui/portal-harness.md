# Portal execution and evidence

## Current integration checkpoint — 15 Sep 2026, #143

The active task contract is [work/0143-midterm-vertical/task.md](../work/0143-midterm-vertical/task.md).
Demo remains isolated and illustrative. Connected uses the canonical evidence
API, actual server identity/template/policy, bounded polling and the approved
limited ClaimRequest submission endpoint. It never falls back to stories.
Terminal authority and final task/cleanup outcome are separate: the page keeps
refreshing until the outcome arrives, and cleanup failure does not hide a
successful model result. Tool/Memory configuration is not represented as live
use. Runtime labels are human-readable; raw records remain expandable.

Focused connected tests intercept canonical API responses and make no provider
or cluster calls. Actual browser-to-kind inference belongs to the separate
release gate. The sections below preserve the earlier preview design/evidence,
not the current connected implementation status.

## Historical portal demo slice — 14 Sep 2026
